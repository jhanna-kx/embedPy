...
ZV*t;EXP K2(init){ZI i=0;I f,g,ifd,nfd;S l,h;K n,v;P a,b;P(i,0)l=zs(x),h=zs(y);f=pyl(l);free(l);P(!f,krr("libpython"))
 Py_SetPythonHome(Py_DecodeLocale(h,0));free(h);
 P(stdinred(&ifd,&nfd),krr("redirect1"));Py_InitializeEx(0);P(stdinres(ifd,nfd),krr("redirect2"));if(PyEval_ThreadsInitialized())t0(PyGILState_GetThisThreadState());PyEval_InitThreads();
